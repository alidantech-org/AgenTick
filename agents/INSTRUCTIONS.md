# Skillib Development Instructions
...
- Never require a root `AGENTS.md` or root Skillib YAML file.
- `skillib init` must preserve existing files and append only its marked `.gitignore` block.
- Keep `packages/skillib/src/tool/` shallow and grouped by human workflow.
