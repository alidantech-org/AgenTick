...
async function send(message: MailMessage): Promise<void> {
  ...
  const from = process.env.EMAIL_FROM ?? "Skillib <onboarding@resend.dev>";
  ...
  if (!apiKey) {
    ...
    console.info(
      `[Skillib mail] ${message.subject} -> ${message.to}\n${message.text}`,
    );
    return;
  }
