...
use std::{collections::HashMap, io::{BufReader, BufWriter}, path::PathBuf};

pub fn serve() -> anyhow::Result<()> {
    let stdin = std::io::stdin();
    let stdout = std::io::stdout();
    let mut transport = StdioTransport::new(BufReader::new(stdin.lock()), BufWriter::new(stdout.lock()));
...
impl Server {
    fn handle(&mut self, message: Value) -> Vec<Value> {
        ...
        match method {
            ...
            "initialized" => Vec::new(),
            "shutdown" => { self.shutdown = true; vec![reply(id, Value::Null)] }
            "exit" => { self.shutdown = true; Vec::new() }
            ...
            "textDocument/hover" => vec![reply(id, hover(&message))],
    ...
    fn did_open(&mut self, message: &Value) -> Vec<Value> {
        ...
        self.documents.insert(uri.clone(), Document {
            path: uri_path(&uri),
            version: item["version"].as_i64().unwrap_or_default() as i32,
    ...
    fn did_change(&mut self, message: &Value) -> Vec<Value> {
        let uri = message["params"]["textDocument"]["uri"].as_str().unwrap_or_default().to_owned();
        let text = message["params"]["contentChanges"].as_array()
            .and_then(|changes| changes.last()).and_then(|change| change["text"].as_str()).unwrap_or_default();
        if let Some(document) = self.documents.get_mut(&uri) {
            document.text = text.to_owned();
            document.version = message["params"]["textDocument"]["version"].as_i64().unwrap_or_default() as i32;
        }
        vec![self.diagnostics(&uri)]
    }
    ...
    fn diagnostics(&self, uri: &str) -> Value {
        let diagnostics = self.documents.get(uri).and_then(|document| SkillibAnalysis.analyze(document).ok())
            .map(|result| result.diagnostics.into_iter().map(|item| diagnostic_json(self.documents.get(uri).unwrap(), item)).collect::<Vec<_>>())
            .unwrap_or_default();
    ...
    fn symbols(&self, message: &Value) -> Value {
        ...
        let Some(document) = self.documents.get(uri) else { return json!([]); };
        Value::Array(document.text.lines().enumerate().filter_map(|(line, text)| {
            let name = text.trim_end_matches(':').trim();
            let top_level = !text.starts_with(' ') && !name.is_empty();
            top_level.then(|| json!({"name":name,"kind":3,"range":range(line,0,line,text.len()),"selectionRange":range(line,0,line,text.len())}))
        }).collect())
    }
    ...
    fn formatting(&self, message: &Value) -> Value {
        let uri = message["params"]["textDocument"]["uri"].as_str().unwrap_or_default();
        let Some(document) = self.documents.get(uri) else { return json!([]); };
        let Ok(source) = skillib_source::SourceFile::new(document.path.clone(), document.text.clone()) else { return json!([]); };
...
fn completions() -> Value {
    Value::Array(SkillibAnalysis.top_level_completions().iter().map(|label| json!({"label":label,"kind":14})).collect())
}

fn hover(message: &Value) -> Value {
    let _ = message;
    json!({"contents":{"kind":"markdown","value":"Skillib declarative language. Use `skillib check` for compiler diagnostics."}})
}

fn diagnostic_json(document: &Document, item: skillib_diagnostics::Diagnostic) -> Value {
    ...
    let severity = match item.severity { skillib_diagnostics::Severity::Error => 1, skillib_diagnostics::Severity::Warning => 2, skillib_diagnostics::Severity::Information => 3 };
...
fn offset_position(text: &str, offset: usize) -> (usize, usize) {
    ...
    let character = prefix.rsplit('\n').next().map(str::chars).map(Iterator::count).unwrap_or_default();
    (line, character)
}

fn range(sl: usize, sc: usize, el: usize, ec: usize) -> Value { json!({"start":{"line":sl,"character":sc},"end":{"line":el,"character":ec}}) }
fn uri_path(uri: &str) -> PathBuf { PathBuf::from(uri.strip_prefix("file://").unwrap_or(uri)) }
fn reply(id: Option<Value>, result: Value) -> Value { json!({"jsonrpc":"2.0","id":id,"result":result}) }
fn error(id: Option<Value>, code: i64, message: &str) -> Value { json!({"jsonrpc":"2.0","id":id,"error":{"code":code,"message":message}}) }
