...
pub fn watch(root: &Path) -> notify::Result<WatchSession> {
    ...
    let session_id = Utc::now().timestamp_nanos_opt().unwrap_or_default().to_string();
    ...
    Ok(WatchSession { _watcher: watcher, events: receiver, session_id })
}

fn debounced(seen: &Mutex<HashMap<PathBuf, Instant>>, path: &Path, delay: u64) -> bool {
    let mut values = seen.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
    let now = Instant::now();
    let duplicate = values.get(path).is_some_and(|last| now.duration_since(*last) < Duration::from_millis(delay));
    values.insert(path.to_path_buf(), now);
    duplicate
}

fn classify(kind: &EventKind, protected: bool) -> String {
    if protected { return "protected_file.changed".into(); }
    match kind {
...
fn build_event(session: &str, kind: String, relative: PathBuf, absolute: &Path) -> WatchEvent {
    if is_skillib_path(absolute) && absolute.is_file()
        ...
        && let Ok(source) = SourceFile::new(absolute.to_path_buf(), text)
    {
        ...
        let skill_kind = if result.success { "skill.valid" } else { "skill.invalid" };
        let mut event = WatchEvent::new(session, skill_kind, Some(relative), format!("{} diagnostic(s)", result.diagnostics.len()));
        event.data = serde_json::json!({"diagnostics": result.diagnostics, "irHash": result.ir_hash});
        return event;
    }
