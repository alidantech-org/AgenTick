use crate::{
    Identity, PackageSummary, PublishRequest, PublishResult, RegistryClient, RegistryError,
    RegistryIndex, ResolvedRelease,
};
...
impl RegistryClient for HttpRegistryClient {
    ...
    async fn index(&self, revision: Option<u64>) -> Result<RegistryIndex, RegistryError> {
        let response = self
            .client
            .get(self.endpoint("api/v1/registry/index")?)
    ...
    async fn resolve(
        ...
        let request = self
            .client
            .post(self.endpoint("api/v1/skills/resolve")?)
    ...
    async fn publish(
        ...
        let response = self
            .client
            .post(self.endpoint("api/v1/skills/publish")?)
            .bearer_auth(token)
            .json(request)
            .send()
            .await?;
