...
async fn main() -> Result<()> {
    ...
    let root = root::discover(&current)?;
    match Cli::parse().command {
        Command::Init => project::init(&root),
        Command::New { name, extension } => {
            let path = language::new_skill(&root, &name, &extension)?;
        ...
        Command::Install { package, selector } => {
            registry_cmd::install(&root, &package, &selector).await
        }
        ...
        Command::Publish {
            ...
            package,
            visibility,
        } => registry_cmd::publish(&root, &file, &package, &visibility).await,
