...
pub fn init(root: &Path) -> Result<()> {
    ...
    write_once(
        &agents.join("ARCHITECTURE.md"),
        "# Architecture\n\nDescribe the project architecture and invariants here.\n",
    )?;
    write_once(&agents.join("skillib.yml"), config())?;
...
pub fn verify(root: &Path) -> Result<()> {
    ...
    anyhow::ensure!(
        root.join("agents/skillib.yml").is_file(),
        "agents/skillib.yml is missing"
    );
...
pub fn history(root: &Path, limit: usize) -> Result<()> {
    ...
    for event in HistoryStore::project(root).recent(limit)? {
...
pub fn status(root: &Path) -> Result<()> {
    ...
    ui::info(format!(
        "events       {}",
        HistoryStore::project(root).recent(usize::MAX)?.len()
    ));
